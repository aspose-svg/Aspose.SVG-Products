...
title: 将 ICO 转换为 SVG - C# 代码和在线矢量化器
description: 在 C# 中矢量化位图。将 ICO 转换为 SVG 并获得矢量图形的所有优势。免费试用在线图像矢量化器！
...
[Aspose.SVG for .NET API](https://products.aspose.com/svg/{{lang.url-fragment}}net/) 提供了一个高速 C# 库，可用于不同的 SVG 解析任务. [ImageVectorization](https://reference.aspose.com/svg/net/aspose.svg.imagevectorization/) 命名空间包括用于实现图像矢量化过程的类和接口。在本文中，我们将了解如何使用 Aspose.SVG API 将 ICO 转换为 C# 中的矢量图形。更重要的是，我们提供了一个在线 Image Vectorizer 供您尝试实时图像到矢量的转换。
...
---
h2: 如何在 C# 中矢量化 ICO
title: 在 C# 中将 ICO 转换为 SVG 的步骤
---

要使用 Aspose.SVG 对 ICO 图像进行矢量化，您应该遵循以下几个步骤：
...
1. 从指定文件矢量化 ICO。 [Vectorize()](https://reference.aspose.com/svg/net/aspose.svg.imagevectorization/imagevectorizer/vectorize/) 方法获取图像文件的路径并返回一个 SVGDocument。
1. 将矢量化 ICO 图像保存为 SVG 文件。使用 Save() 方法并将输出路径传递给它。
