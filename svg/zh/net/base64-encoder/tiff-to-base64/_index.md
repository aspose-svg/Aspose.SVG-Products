...
title: 将 TIFF 转换为 Base64 - 在线编码器和 C# 代码
description: 在线或在 C# 中将 TIFF 转换为 Base64，并使用编码字符串作为数据 URI。 将其嵌入 HTML、CSS、XML、JSON 等。
...
{{<section banner>}}
---
h1: 在线或使用 C# 将 TIFF 转换为 Base64
h2: 在 C# 中将 TIFF 编码为 Base64 字符串并将其用作数据 URI 的示例
---
...
由于 Base64 编码数据的大小增加了 33%，因此建议仅对小图像使用数据 URI。大型 Base64 图像会在 HTML 中生成大量代码，这会导致性能优势的损失。如果您需要将 TIFF 转换为 Base64 并使用编码字符串嵌入到 HTML、CSS、JSON 或 SVG 文件中，请先了解所有优缺点。

{{<section code-text>}}
---
h2: 如何在 C# 中将 TIFF 转换为 Base64
title: 将 TIFF 转换为 Base64 字符串并将其嵌入到 SVG 文件中的 C# 代码
---

为了将 TIFF 转换为 Base64 字符串，我们使用了 [Aspose.SVG for .NET](https://products.aspose.com/svg/net/) API，它是一个功能丰富、功能强大、易于使用的 API。使用 C# 平台的文档操作 API。我们考虑 TIFF 到 Base64 编码并将 Base64 字符串作为数据 URI 嵌入到 SVG 文档中的示例。只需几行代码即可完成这些操作：
...
{{<section steps>}}
---
h2: 在 C# 中将 TIFF 转换为 Base64 的步骤
---
1. 打开要转换的 TIFF 图像。使用 ReadAllBytes(`path`) 方法打开 TIFF 图像并将文件内容读入字节数组。
...
1. 将 TIFF 转换为 Base64。调用 ToBase64String(bytes) 方法将 8 位整数数组转换为以 base64 数字编码的等效字符串表示形式。
...
---
h2: Base64 编码使用示例
svg: 将 Base64 TIFF 图像嵌入 SVG 文档的 SVG 代码
html: 嵌入 Base64 TIFF 图像的 HTML 代码
css: 将 Base64 TIFF 图像嵌入为背景图像的 CSS 代码
xml: 将 Base64 TIFF 图像嵌入 XML 文档的 XML 代码
json: 将 Base64 TIFF 图像嵌入 JSON 文档的 JSON 代码
---
