...
title: Объединение PNG в BMP через C# - Aspose.SVG
...
{{<section banner>}}
---
h1: Слияние PNG с BMP через C#
h2: Высокоскоростной .NET API для объединения файлов PNG в Windows, macOS и Linux.
---

{{<section overview>}}
---
h2: Объединение PNG в BMP с помощью C#
---

PNG относится к типу формата файла растрового изображения, который использует сжатие без потерь, что делает его популярным среди пользователей. Графические файлы в формате BMP представляют собой несжатые растровые изображения, поэтому изображения BMP имеют более высокое разрешение, чем PNG. Оба формата файлов имеют свои плюсы и минусы, и в некоторых случаях вам может понадобиться объединить изображения PNG и сохранить их как один файл BMP. С помощью API [Aspose.SVG для .NET](https://products.aspose.com/svg/net/) вы можете программно объединить PNG в BMP. Мощный C# API поможет вам быстро и качественно объединить файлы PNG!

{{<section code-text>}}
---
h2: Пример кода C# для объединения PNG в BMP
title: Объединение PNG в BMP — C#
---

Объедините несколько файлов PNG и легко сохраните результат в виде одного файла BMP! Объединение изображений можно выполнить с помощью нескольких строк кода:
...
{{<section steps>}}
---
h2: 'Шаги по объединению PNG в BMP с помощью C #'
---
...
1. Несколько изображений PNG будут сохранены в один файл BMP по указанному пути.
